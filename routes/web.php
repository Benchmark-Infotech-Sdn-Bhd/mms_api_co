            ...
            $router->group(['prefix' => 'onboarding'], function () use ($router) {
                ...
                $router->group(['prefix' => 'callingVisa'], function () use ($router) {
                    $router->group(['prefix' => 'process'], function () use ($router) {
                        ...
                        $router->post('updateCallingVisa', 'V1\DirectRecruitmentCallingVisaController@updateCallingVisa');
                    ...
                    $router->group(['prefix' => 'approval'], function () use ($router) {
                        $router->post('callingVisaStatusUpdate', 'V1\DirectRecruitmentCallingVisaApprovalController@callingVisaStatusUpdate');
                        $router->post('workersListForApproval', 'V1\DirectRecruitmentCallingVisaController@workersListForApproval');
                    });
                });
            });
        });
