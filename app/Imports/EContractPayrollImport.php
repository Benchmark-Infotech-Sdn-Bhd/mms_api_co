...
use App\Jobs\EContractPayrollsImport;
use Exception;
...
class EContractPayrollImport implements ToModel, WithChunkReading, WithHeadingRow
{
    ...
    /**
     * Process a row of data for the model.
     *
    ...
    public function model(array $row)
    {
        try {
            ...
            dispatch(new EContractPayrollsImport($payrollParameters, $this->bulkUpload));
    ...
    /**
     * Increment the total records of a specific bulk upload.
     *
     * @return void
     */
    private function incrementTotalRecords()
    {
        DB::table('e-contract_payroll_bulk_upload')->where('id', $this->bulkUpload->id)->increment('total_records');
    }

    /**
     * Get the payroll parameters for a given row.
     *
     * @param array $row The row containing the payroll information.
     * @return array The payroll parameters.
     */
    private function getPayrollParameters(array $row)
    {
        return [
